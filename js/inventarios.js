...
function agregar_mercaderia(numero_mercaderia, id_producto) {
    deshabilitar_para_transaccion();
    $.post('./modulos/inventario/agregar_mercaderia_producto.php', {
        "numero_mercaderia": numero_mercaderia,
        "id_producto": id_producto
    }, function (respuesta) {
        respuesta = JSON.parse(respuesta);
        if (respuesta === true) {
            $("#mostrar_resultados_agregar_mercaderia")
                .html("<i class='fa-check fa'></i> ¡Mercaderia agregada correctamente!")
                .parent()
                .removeClass('alert-success alert-warning')
                .addClass('alert-success')
                .delay(300)
                .hide("slow", function () {
                    $("#modal_agregar_mercaderia").modal("hide");
                    $("#numero_mercaderia_agregar").val("");
                    $("#agregar_mercaderia").prop("disabled", false);
                });
            consultar_todos_los_productos();
        }
        habilitar_para_transaccion();
...
function llenar_formulario_para_editar(datos_producto) {
    ...
    $("#proveedor").val(datos_producto.proveedor);
...
function escuchar_elementos() {
    ...
    $(document).on("click", ".eliminar, .editar, .agregar-mercaderia, .dar-baja", function () {
    ...
    $("#modal_agregar_mercaderia").on("hidden.bs.modal", function () {
        $("#numero_mercaderia_agregar").val("").parent().removeClass('has-error');
    });

    $("#modal_dar_baja").on("shown.bs.modal", function () {
        $("#numero_mercaderia_baja").focus();
    });

    $("#modal_dar_baja").on("hidden.bs.modal", function () {
        $("#numero_mercaderia_baja, #motivo_baja").val("");
        $("#mostrar_resultados_dar_baja").parent().hide();
        $("#numero_mercaderia_baja").parent().removeClass('has-error');
    });

    $("#modal_agregar_mercaderia").on("shown.bs.modal", function () {
        $("#numero_mercaderia_agregar").focus();
    });
    ...
    $(document).on("click", ".agregar-mercaderia", function (evento) {
        id_temporal_ayudante = $(this).data("id");
        $("#modal_agregar_mercaderia").modal("show");
    });

    $("#numero_mercaderia_agregar").keyup(function (evento) {
        if ($(this).val().length > 0) $(this).parent().removeClass('has-error');
    });

    $("#agregar_mercaderia").click(function (evento) {
        var numero_mercaderia = $("#numero_mercaderia_agregar").val();
        if (isNaN(numero_mercaderia) || numero_mercaderia.length <= 0 || numero_mercaderia <= 0) {
            $("#numero_mercaderia_agregar").focus().animateCss("shake");
            $("#numero_mercaderia_agregar").parent().addClass('has-error');
            return;
        }
        $("#mostrar_resultados_agregar_mercaderia")
            .html("<i class='fa fa-spinner fa-spin'></i> Agregando mercaderia...")
            .parent()
            .removeClass('alert-success alert-warning')
            .addClass('alert-warning')
            .show("slow");
        $(this).prop("disabled", true);
        agregar_mercaderia(numero_mercaderia, id_temporal_ayudante);
    });
    ...
    $("#numero_mercaderia_baja").keyup(function () {
    ...
    $("#dar_baja").click(function () {
        var numero_mercaderia = $("#numero_mercaderia_baja").val(),
            razon = $("#motivo_baja").val();
        var mensaje_error = undefined;
        if (numero_mercaderia === "")
            mensaje_error = "No puedes dejar el campo vacío.";
        if (numero_mercaderia <= 0)
            mensaje_error = "No puedes quitar menos de cero mercaderia.";
        if (ayudante_cantidad - numero_mercaderia < 0)
            mensaje_error = "No puedes quitar más mercaderia de las que tienes.";
        if (mensaje_error !== undefined) {
            $("#mostrar_resultados_dar_baja")
                ...
                .addClass('alert-warning')
                .show("slow", function () {
                    $("#numero_mercaderia_baja").focus().animateCss("shake");
                    $("#numero_mercaderia_baja").parent().addClass("has-error");
        ...
        $("#numero_mercaderia_baja, #motivo_baja").parent().removeClass('has-error');
        $("#mostrar_resultados_dar_baja")
            ...
            .removeClass('alert-success alert-warning alert-danger')
            .addClass('alert-warning');
        dar_baja(numero_mercaderia, razon);
    });
                                ...
                                $("<button data-toggle='tooltip'>")
                                    ...
                                    .tooltip({
                                        ...
                                        title: $("<div>")
                                            ...
                                            .append(
                                                $("<button data-id = '" + productos[i].rowid + "'>")
                                                    .addClass('agregar-mercaderia btn btn-success')
                                                    .html(
                                                        $("<i>")
