<?php
function consultar_todas_las_ventas_por_proveedor($fecha_inicio, $fecha_fin)
{
...
function consultar_todas_las_ventas($fecha_inicio, $fecha_fin, $proveedor)
{
    global $base_de_datos;
    if ($_SESSION["administrador"] === 1) {
        if ($proveedor === "*") {
            ...
            $sql = "SELECT * FROM ventas WHERE fecha > ? AND fecha < ? AND proveedor = ? ORDER BY numero_venta DESC;";
        }
    } else {
        if ($proveedor === "*") {
            $sql = "SELECT * FROM ventas WHERE fecha > ? AND fecha < ? AND usuario = ? ORDER BY numero_venta DESC;";
        } else {
            $sql = "SELECT * FROM ventas WHERE fecha > ? AND fecha < ? AND proveedor = ? AND usuario = ? ORDER BY numero_venta DESC;";
        }
    }
    $sentencia = $base_de_datos->prepare($sql);
    if ($_SESSION["administrador"] === 1) {
        if ($proveedor === "*") {
            ...
            $sentencia->execute([$fecha_inicio, $fecha_fin, $proveedor]);
        }
    } else {
        if ($proveedor === "*") {
            $sentencia->execute([$fecha_inicio, $fecha_fin, $_SESSION["nombre_de_usuario"]]);
        } else {
            $sentencia->execute([$fecha_inicio, $fecha_fin, $proveedor, $_SESSION["nombre_de_usuario"]]);
        }
    }
    return $sentencia->fetchAll();
}
...
function hacer_venta($productos, $total, $ticket, $cambio)
{
    ...
    foreach ($productos as $producto) {
        $todo_correcto = $todo_correcto and quitar_mercaderia($producto->cantidad, $producto->rowid);
        $sentencia = $base_de_datos->prepare("INSERT INTO ventas(numero_venta, codigo_producto, nombre_producto, total, fecha, numero_productos, usuario, proveedor, utilidad) VALUES (?,?,?,?,?,?,?, ?, ?);");
        $resultado_sentencia = $sentencia->execute(array($numero_venta, $producto->codigo, $producto->nombre, $producto->cantidad * $producto->precio_venta, date("Y-m-d H:i:s"), $producto->cantidad, $_SESSION["nombre_de_usuario"], $producto->proveedor, $producto->utilidad * $producto->cantidad));
